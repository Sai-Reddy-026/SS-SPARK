...
export const AnalyzerSidebar = memo(function AnalyzerSidebar({
  ...
  return (
    <aside
      className={cn(
        "z-30 flex h-full shrink-0 flex-col overflow-hidden border-r bg-sidebar/80 backdrop-blur-xl transition-[width] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]",
    ...
    >
        ...
        <Link to="/" className="flex min-w-0 items-center gap-2.5">
          ...
          {open && (
            <span className="min-w-0">
              <span className="block truncate text-sm font-bold tracking-tight">SS SPARK</span>
      ...
      <div className="px-3">
        <Button
          onClick={onNewChat}
          className={cn("w-full gradient-brand text-brand-foreground hover-lift group", !open && "px-0")}
        >
          <MessageSquarePlus className="h-4 w-4 transition-transform group-hover:rotate-90" />
      ...
      {open && (
        <div className="relative mt-3 px-3">
          ...
          <Input
            ...
            className="h-9 bg-background/50 pl-9 text-sm premium-focus"
          />
        </div>
      )}
          ...
          <div className="space-y-1">
            {filteredChats.map((chat) => (
              <button
                ...
                title={chat.title}
                className={cn(
                  "relative flex w-full items-center gap-2 rounded-xl px-2.5 py-2 text-left text-sm transition-all duration-200",
                  activeChat === chat.id
              ...
              >
                {activeChat === chat.id && (
                  <span className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-0.5 rounded-full gradient-brand" />
                )}
                ...
                {open && (
                  <span className="min-w-0 flex-1">
                    ...
                    {chat.subtitle && (
                      <span className="block truncate text-[11px] opacity-60">{chat.subtitle}</span>
                    )}
                  </span>
                )}
              </button>
      ...
      <div className="mt-auto space-y-1 border-t p-3">
        ...
        <button
          onClick={onOpenSettings}
          className={cn(
            "flex w-full items-center gap-2.5 rounded-xl px-2.5 py-2.5 text-left transition-all duration-200 hover:bg-sidebar-accent/60 cursor-pointer group",
            !open && "justify-center",
          )}
        >
          {user?.avatar_url ? (
            <img
              src={user.avatar_url}
              alt={userName}
              className="h-8 w-8 shrink-0 rounded-full border border-border/50 object-cover transition-transform group-hover:scale-105"
            />
          ) : (
            <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full gradient-brand text-xs font-semibold text-brand-foreground shadow-sm transition-transform group-hover:scale-105">
              {initials}
            </span>
          )}
          {open && (
